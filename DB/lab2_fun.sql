...
CREATE TABLE Chikunov.Cars (
	...
	-- CHECKS
	CONSTRAINT CH_Cars_number CHECK (number LIKE '[АВЕКМНОРСТУХABEKMHOPCTYX][0-9][0-9][0-9][АВЕКМНОРСТУХABEKMHOPCTYX][АВЕКМНОРСТУХABEKMHOPCTYX]'),
	CONSTRAINT CH_Cars_region_code CHECK (region_code < 100 OR region_code BETWEEN 100 AND 199 OR region_code BETWEEN 200 AND 299 OR region_code BETWEEN 700 AND 799)
)
...
GO
CREATE FUNCTION Chikunov.lastAction (@car_id int, @rec_id int)
RETURNS int
AS
BEGIN
    DECLARE @result bit = (SELECT TOP(1) incoming FROM Chikunov.Records WHERE id != @rec_id AND car_id = @car_id ORDER BY record_time DESC)
	IF @result IS NULL RETURN -1
	RETURN CAST(@result AS int)
END

GO
ALTER TABLE Chikunov.Records ADD CONSTRAINT CH_Records CHECK(incoming != Chikunov.lastAction(car_id, id))
go 
